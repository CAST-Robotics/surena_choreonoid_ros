...
class robot_sim:
    def __init__(self, render, robot_vel = 0.7, time = 5.0, real_time = False, freq = 240.0):
        ...
        self.optimServer = rospy.Service('walk_srv', Walk, self.run)
    ...
    def run(self, optim_req):
        ...
        step_count = math.floor(self.simTime/optim_req.t_step)
        size = int(((step_count + 2) * optim_req.t_step + 1) * self.freq)

        done = trajectory_handle(optim_req.alpha,optim_req.t_ds_ratio * optim_req.t_step,optim_req.t_step,
                    optim_req.t_step * self.robotVel,optim_req.COM_height, step_count, optim_req.ankle_height, 1 / self.freq)
        
        while not done:
            print("Trajectory generation failed, calling again...")
            done = trajectory_handle(optim_req.alpha,optim_req.t_ds_ratio * optim_req.t_step,optim_req.t_step,
                    optim_req.t_step * self.robotVel,optim_req.COM_height,math.floor(self.simTime/optim_req.t_step), optim_req.ankle_height, 1 / self.freq)
        if done:
